...
mod tests {
    ...
    fn bridge_is_available_false_for_missing_binary() {
        ...
        let bridge =
            crate::kanban::bridge::AdvancedBridge::new(PathBuf::from("/nonexistent/hermes-binary-xyz"), tmp.clone());
    ...
    fn specify_fails_gracefully_when_hermes_missing() {
        ...
        let task_id = store
            .create_task(crate::kanban::CreateTaskRequest {
                board_id,
        ...
        let tmp = std::env::temp_dir().join(format!("iota-bridge-{}", uuid::Uuid::new_v4()));
        std::fs::create_dir_all(&tmp).unwrap();
        let bridge = crate::kanban::bridge::AdvancedBridge::new(PathBuf::from("/nonexistent/hermes-xyz"), tmp.clone());
    ...
    fn with_timeout_configures_bridge() {
        let tmp = std::env::temp_dir().join(format!("iota-bridge-{}", uuid::Uuid::new_v4()));
        std::fs::create_dir_all(&tmp).unwrap();
        let bridge = crate::kanban::bridge::AdvancedBridge::new(PathBuf::from("hermes"), tmp.clone())
    ...
    fn specify_respects_timeout() {
        ...
        let bridge = crate::kanban::bridge::AdvancedBridge::new(hermes_path, tmp.join("shadows"))
    ...
    fn read_new_shadow_tasks_excludes_existing_materialized_tasks() {
        ...
        let tasks = crate::kanban::bridge::read_new_shadow_tasks(&db_path, &["1".to_string(), "2".to_string()]).unwrap();
