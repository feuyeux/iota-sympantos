...
OTLP 导出默认关闭，CLI 默认只初始化 stderr tracing，不要求本机运行 collector。如需导出 trace、metric、log，设置 `OTEL_ENABLED=true` 使用默认 `http://localhost:4317`，或设置 `OTEL_EXPORTER_OTLP_ENDPOINT` 显式指定 collector endpoint；`OTEL_ENABLED=false` 可强制关闭导出。

## Local Stores

`CacheStore` 位于 `crates/iota-core/src/store/cache.rs`，当前只负责 execution lifecycle：
...
`ObservabilityStore` 位于 `crates/iota-core/src/store/observability.rs`，同样使用 `~/.i6/context/events.sqlite`，负责 token usage 持久化和查询：
