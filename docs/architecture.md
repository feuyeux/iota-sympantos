...
│   ├── observability.rs     # token usage events、execution-level 去重、backend summary
...
| `store/approvals.rs` | approval request/decision 记录、风险维度分类、默认人工审批策略；新增 `get_pending_requests()` 和 `get_decision_history(execution_id, limit)` | `~/.i6/context/approvals.sqlite` |
| `store/ledger.rs` | iota session、backend session、turn、handoff；新增 `session_stats(session_id)` 和 `get_handoff_history(session_id)` 用于会话分析 | `~/.i6/context/sessions.sqlite` |
