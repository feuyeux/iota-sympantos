...
│   ├── cache.rs             # execution replay / join-running / dedupe
...
| 模块 | 职责 | 默认路径 |
|---|---|---|
| `store/cache.rs` | execution replay、join-running、dedupe、fencing token；仅保存 Output events 用于 replay | `~/.i6/context/events.sqlite` |
...
`RuntimeEvent` 随 `AcpPromptOutput.events` 返回；`CacheStore` 只持久化 Output events 以支持 replay。
...
| 新 RuntimeEvent | `runtime_event.rs`, 相关生产方 | 增加事件类型；按需接入 CLI/TUI 输出或 CacheStore replay |
