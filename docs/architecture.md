...
| `store/observability.rs` | token usage events、raw payload、execution-level 去重、backend summary | `~/.i6/context/events.sqlite` |
...
| `store/approvals.rs` | approval request/decision 记录、风险维度分类、默认人工审批策略 | `~/.i6/context/approvals.sqlite` |
