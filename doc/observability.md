...
|------|------|
| `logging` | 浏览执行日志与事件流 |
| `timing` | 检查延迟与 timing 数据 |
| `metrics` | 查看聚合计数与指标 |

数据持久化在 `~/.i6/context/events.sqlite`，自动保留 30 天。实现为纯本地查询：命令打开 `EventStore::default_path()` 通过类型化 Rust API 查询，不暴露任意 SQL。

注意：Observability 的 `logging` 子命令查询的是 execution/event 运行事件流，不是工程日志文件。工程日志由 file appender 写入 `~/.i6/logs/`，用于排查程序自身行为；EventStore 用于查询执行、工具调用、memory、approval 和 timing 等结构化事件。

### OpenTelemetry 对齐状态

当前实现还不能声明为完整 OpenTelemetry 实现。它已经有本地 logging、metrics 和 timing 数据源，但没有 OTLP exporter，也没有使用 OpenTelemetry SDK 建模三类信号。

| 信号 | 当前状态 | OTel 差距 |
|------|----------|-----------|
| Logs | 有工程日志文件、`RuntimeEvent::Log`、`--log-events` 和 `observability logging` 查询 | 不是 OTel LogRecord；缺少 ObservedTimestamp、SeverityNumber、Resource、InstrumentationScope、Body/EventName 的明确映射，也没有 OTLP 日志导出 |
| Metrics | 有 EventStore 聚合和 Prometheus text 输出 | 不是 OTel Metrics SDK/OTLP 数据流；部分 `_total` 指标目前由 gauge 输出，单位和属性模型未系统化 |
| Span 链路 | 只有 `timing` 延迟分解 | 没有 span_id、parent_span_id、SpanKind、Status、Events、Links、上下文传播和 OTLP span 导出 |

结论：现在可以确认的是“本地 observability 基础存在”，不能确认“完整、成体系、符合 OpenTelemetry”。要达到 OTel，需要新增明确的 OTel 数据模型层、SDK/exporter 配置和三类信号的语义映射。
...
- `log` / `log-events` / `metric` 均为对应主命令的别名
...
`--scan N` 控制最近 execution 的扫描窗口，默认至少扫描 500 条 execution，避免 `--tool` 过滤时因稀疏工具调用漏掉可运行事件记录。

### timing — 查看延迟与 timing 数据

```bash
iota observability timing recent [--limit N]        # 近期执行（含 timing 字段）
iota observability timing slow [--limit N]          # 最慢的 N 条执行（按 total_ms DESC）
iota observability timing breakdown <execution-id>  # 单次执行 5 段耗时分解
iota observability timing summary                   # avg / p95 延迟统计
```

`timing breakdown` 输出格式：
```json
{
  "execution_id": "...",
  "backend": "codex",
...
iota observability metrics --prometheus              # Prometheus text exposition 格式
...
```
CLI: iota observability logging/timing/metrics
     |
     ...
     `-> run_observability_command(&args)
         ...
         |
         |-> timing: recent_executions / slowest_executions / get_execution / observability_summary
...
│      │
│      ├─► Console log-events renderer (`iota run --log-events`)
│      │
...
Output Format: Prometheus text exposition
```
...
`LogEvent` 是控制台 log-events、EventStore 运行事件和日志字段的统一结构：
...
cargo run -- observability timing --help
