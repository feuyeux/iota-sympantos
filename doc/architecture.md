...
本文按当前源码组织架构。具体调用链见 [code-call-chains.md](code-call-chains.md)，观测路径见 [observability.md](observability.md)，调试手册见 [debugging.md](debugging.md)。
...
├── telemetry/               # OpenTelemetry providers, instruments, stderr layer
...
| 模块 | 职责 | 主要下游 |
|---|---|---|
| `cli/mod.rs` | 解析命令并分发 `run/check/tui/bench/logs/trace/context-mcp/fun-mcp/native-materialize/skill/__daemon`；负责 daemon autostart、OTel 初始化和 CLI 输出 | `config`, `engine`, `daemon`, `acp`, `native`, `skill`, `tui`, `telemetry` |
...
| 模块 | 职责 | 主要下游 |
|---|---|---|
| `engine.rs` | 核心编排门面。按 `(backend, cwd)` 维护 ACP client pool；处理 replay/join running、session ledger、handoff、memory recall/write、skill 短路、context capsule、ACP 调用、CacheStore 写回和 OTel 指标/日志 | `acp`, `config`, `context`, `skill`, `store`, `runtime_event`, `telemetry` |
...
| 模块 | 职责 | 主要下游 |
|---|---|---|
| `context/mod.rs` | 组装 `<iota-context>` capsule：session/model、memory tools 提示、memory buckets、dialogue、workspace `git status --short`、skill index、handoff | `config`, `store::memory`, `skill` |
...
| `native/mod.rs` | 将 memory/skill 投影到 backend 原生文件，使用 `<!-- IOTA_START -->` / `<!-- IOTA_END -->` 块替换 | `store::memory`, `skill`, `acp::AcpBackend` |
...
| 模块 | 职责 | 默认路径 |
|---|---|---|
| `store/cache.rs` | execution replay/dedupe cache；request hash、running join、fencing token、output replay、30 天 cache retention | `~/.i6/context/events.sqlite` |
| `store/memory.rs` | memory taxonomy、dedup、TTL、merge mode、recall buckets、FTS/LIKE、vector/hybrid search | `~/.i6/context/memory.sqlite` 或 `context_engine.memory_db` |
| `store/embedding.rs` | embedding 计算；engine 按 `context_engine.embedding` 配置 Ollama `/api/embeddings`，失败或未配置时走 128 维本地 trigram fallback | 存入 `memory_embedding` 表 |
...
| 模块 | 职责 |
|---|---|
| `telemetry/mod.rs` | 初始化 OTel tracer/meter/logger provider；默认 OTLP endpoint 为 `http://localhost:4317`；安装 stderr tracing layer |
| `telemetry/metrics.rs` | 定义 execution/cache/token/latency 等 OTel instruments |
| `telemetry/logs.rs` | `LogEvent` 到 OTel attributes 的辅助映射 |
| `telemetry/spans.rs` | execution/phase/tool/memory/approval span helper |
| `telemetry/console.rs` | stderr fmt layer |

Docker observability stack 位于 `docker/observability/`，包含 OTel Collector、Jaeger、Prometheus、Loki 和 Grafana。
...
| `budgets` | memory/skills/dialogue/workspace 字符预算 |
...
当前实现不再把 `RuntimeEvent` 全量写入 SQLite EventStore。`RuntimeEvent` 仍用于 engine/daemon/TUI 的内存返回值和 `iota run --log-events` stderr 输出；持久化观测由 OpenTelemetry 后端负责。

### Observability Signals

| Signal | Producer | Destination |
|---|---|---|
| Logs | `tracing::*` macros and OTel log bridge | stderr plus OTLP logs to `OTEL_EXPORTER_OTLP_ENDPOINT` |
...
There is no current `iota observability` command group and no local `~/.i6/logs` file appender. See [observability.md](observability.md).
...
| 新 RuntimeEvent | `runtime_event.rs`, 相关生产方 | 增加事件类型，并按需要接入 `--log-events`、TUI/daemon 返回值或 OTel 日志/指标 |
