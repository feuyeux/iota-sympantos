...
│   warm control plane          client pool (BTreeMap)│
...
|---|---|---|
| `cli.rs` | 命令分发（check/info/acp/tui/daemon/warm/bench）、参数解析、输出格式化 | agent, engine, config, acp |
...
| `agent.rs` | Daemon TCP 服务器：保持常驻 IotaEngine，接受外部 prompt/warm JSON 请求 | engine, acp, config |
...
```
cli::run()
  → agent::send_prompt(daemon_addr, request)     # try daemon
      → TCP → agent daemon → engine.prompt_in_cwd_timed()
                                → acp::AcpClient::prompt_with_cwd_timed()
  ← DaemonPromptResponse { text, timing }
...
3. **Lazy client start** — TUI 和 fallback 路径只在首次使用 backend 时 spawn 进程，减少启动时间。

4. **Daemon 预热** — `--warm` 在 accept 前并行 spawn 所有 enabled backends，后续 CLI 调用零 spawn 开销。
