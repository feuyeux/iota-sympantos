...
├── cli/
│   └── mod.rs               # 命令分发、daemon autostart、bench、observability、native、skill
...
│   ├── approval.rs          # approval audit log 和默认风险分类
│   ├── embedding.rs         # Ollama API / 本地 trigram embedding
│   ├── events.rs            # execution/event/observability SQLite store
...
| 模块 | 职责 | 主要下游 |
|---|---|---|
| `cli/mod.rs` | 解析命令并分发 `run/check/tui/bench/observability/context-mcp/fun-mcp/native-materialize/skill/__daemon`；负责 daemon autostart 和 CLI 输出 | `config`, `engine`, `daemon`, `acp`, `store`, `native`, `skill`, `tui` |
| `tui.rs` | 终端生命周期、事件循环、prompt 队列、后台 engine task、流式输出、approval 浮层、pager/help/quit overlay | `engine`, `acp::permission`, `store::events`, `tui/*` |
...
| 模块 | 职责 | 主要下游 |
|---|---|---|
| `engine.rs` | 核心编排门面。按 `(backend, cwd)` 维护 ACP client pool；处理 replay/join running、session ledger、handoff、memory recall/write、skill 短路、context capsule、ACP 调用和事件落库 | `acp`, `config`, `context`, `skill`, `store`, `runtime_event` |
...
| `acp/permission.rs` | 处理 `session/request_permission`；`iota_*`、`mcp__iota-*` 或 backend `tool_whitelist` 命中时自动批准；否则走 TUI 或 stdin；记录 approval audit | `store::approval`, `runtime_event` |
...
| 模块 | 职责 | 默认路径 |
|---|---|---|
| `store/events.rs` | executions/events/observability SQLite；request hash replay、running join、fencing token、timing、Prometheus metrics | `~/.i6/context/events.sqlite` |
...
| `store/approval.rs` | approval request/decision audit、风险维度分类、默认人工审批策略 | `~/.i6/context/approvals.sqlite` |
...
所有 ACP、skill、memory、approval 和 timing 路径最终都通过 `RuntimeEvent` 进入 `EventStore`，并被 `observability` 命令读取。
...
| 新 RuntimeEvent | `runtime_event.rs`, `store/events.rs`, 相关生产方 | 增加事件类型和落库/观测读取 |
